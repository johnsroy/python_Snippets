...
########NEED TO ADD THIS API TO MY LOCATION PORTION#################
#import urllib, json
#url = "http://maps.googleapis.com/maps/api/geocode/json?address=google"
#url = "https://classroom.udacity.com" #####HTTPS cannot be opened easily.. Check ur bookmarks to fig out how to
...
##### Works for most HTTP/ HTTPS web pages but not for d2l.ucalgary.ca########
##### Also suggested by Udacity ###########
'''
from BeautifulSoup import BeautifulSoup
import urllib2
import re

html_page = urllib2.urlopen("https://d2l.ucalgary.ca/d2l/home/106635")
...
#    while True:
#        url,endQuote = get_next_target(page)
#        if url:
#            print url
#            page = page[endQuote:]
#        else:
...
############################################ SEARCH ENGINE BEGINS #####################################################
 add_to_index not repeating the urls
def add_to_index(index,keyword,url):
    for entry in index:
        if entry[0] == keyword:
            if not url in entry[1]: #TO ADD THE URL ONLY ONCE FOR A KEYWORD AND NOT ADD THE SAME URL TO THE KEYWORD MULTIPLE TIMES
                entry[1].append(url)
            return
    index.append([keyword,[url]])

#### ADD TO INDEX SHOWING THE REPEATATION COUNT ###

###### ADD TO INDEX WITHOUT DICTIONARY ########
def add_to_index(index,keyword,url):
    # format of index: [[keyword, [[url, count], [url, count],..]],...]
    for entry in index:
        if entry[0] == keyword:
            for urls in entry[1]:
                if urls[0] == url:  ## CHECKS IF MATCHES , DOESN'T INCREMENT
                    return
            entry[1].append([url,0])
            return
    index.append([keyword,[[url,0]])

########ADD TO INDEX   WITH DICTIONARY#############
def add_to_index(index,keyword,url):
    if keyword in index:
        #if keyword found
...
################ COUNTING THE NUMBER OF URL CLICKS################
...
def crawl_web(seed,max_depth):
    ...
    while tocrawl and depth<=max_depth:
        ...
        if page not in crawled:
            ...
            crawled.append(page)
        if not tocrawl:         #nothing more to crawl at that level, tocrawl must move to the next level
            tocrawl, next_depth = next_depth, [] #next_depth becomes empty again and tocrawl has another level to crawl
            depth = depth + 1
    return index

 ##############   RETURNING A GRAPH AND THE INDEX   ###########
...
########################       RANKING BASED ON NUMBER OF USER CLICKS    ##############################
...
#print crawl_web('http://xkcd.com/353',5) ##For maximum number of pages use this to run
...
url = "https://d2l.ucalgary.ca/d2l/home/106635" + input
...
import httplib
conn = httplib.HTTPSConnection("d2l.ucalgary.ca")
conn.request("GET", "/")
r1 = conn.getresponse()
print r1.status, r1.reason
'''
##############################################   READ THE CODE ABOVE AND THE FINAL CODE IS BELOW ######################################
...
### QUICK SORT #####
#### ORDERS THE RESULT ACCORDING TO THE RANK #####
...
# For simplicity, we use the first element in the list as our pivot element (this
...
def quicksort(pages,ranks):
    if not pages or len(pages) < 1: #edge cases, no url or no. of urls less than 1
        return pages
    else:
        pivot = ranks[pages[0]]
        worse = []
